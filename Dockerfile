# Compile the standalone `mdio` client binaries (one per shipped platform)
...
ENV MDIO_VAULT=/data/vault
RUN printf '# Welcome to mdio\n\nThis vault is empty — start writing, or point an MCP agent here.\n' \
      > /app/welcome-seed.md

EXPOSE 4321
CMD ["sh", "-c", "mkdir -p \"$MDIO_VAULT\" && [ -n \"$(ls -A \"$MDIO_VAULT\")\" ] || cp /app/welcome-seed.md \"$MDIO_VAULT/welcome.md\"; exec bun run src/server/index.ts"]
